...
class UserCtrl extends Base {
  ...
  async payNotifyAPI(ctx, next) {
    ...
    //   const ChargeAmt = (await db.order.find({ '_id': result.out_trade_no })).total_fee
      const payQuery = {
        attach: 'ccb8772a-7bbc-461f-8864-5a69efe637ff'
      }
      const ChargeAmt = 0.01
      const data = 
      `<![CDATA[
        <HisTrans>
      ...
      if (!rechargeResult.RespCode) {
        await db.order.findOneAndUpdate(
          { '_id': result.out_trade_no },
          { isPay: true }
        )
      }
