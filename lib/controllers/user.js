...
class UserCtrl extends Base {
  ...
  async setWithHisAccountMsg(ctx) {
    ...
    for(let account of result) {
      console.log("aaaa",_.padStart(account.healthNo, 8))
      const data = 
        `<![CDATA[
          <HisTrans>
          ...
          <AccountNo>${ _.padStart(account.healthNo, 8) }</AccountNo>
          <Tel></Tel>
          <Name></Name>
          </HisTrans>
        ]]>`
  ...
  async getDoctorsAPI(ctx) {
    ...
    const data = 
      `<![CDATA[
        ...
        <QueryEndDate>${ moment().add(14,'d').format('YYYYMMDD') }</QueryEndDate>
    ...
    await this.setWithDoctorWorkDate(ctx)
  }
