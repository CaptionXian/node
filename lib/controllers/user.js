...
class UserCtrl extends Base {
  ...
  async setWithDoctorWorkDate(ctx) {
    ...
    for (let doctor of result) {
      ...
      `<![CDATA[
        ...
        <QueryDate>${ moment().format('YYYYMMDD') }</QueryDate>
        <DevNo></DevNo>
        <DevSeqNo></DevSeqNo>
        <Date>${ date ? date : '' }</Date>
        <Time>${ Time ? Time : '' }</Time>
  ...
  //  门诊预交金充值
  async rechargeHisAccountAPI(ctx) {
    ...
    res.out_trade_no = obj.out_trade_no
    ctx.body = res
  }

  // 支付结果通知
  async payNotifyAPI(ctx) {
    ...
    //  告知微信已收到，并调 HIS 接口充值
    if (result.return_code && result.return_code == 'SUCCESS' && result.trade_state == 'SUCCESS' ) {
      const ChargeAmt = (await db.order.findOne({ '_id': ObjectId(result.out_trade_no) })).total_fee
      const isPay = (await db.order.findOne({ '_id': ObjectId(result.out_trade_no) })).isPay
      if (!isPay) {
        const data = 
        `<![CDATA[
          <HisTrans>
        ...
        const rechargeResult = await xmlUtil.handleXml(data, true)
        if (rechargeResult.RespCode === '0') {
          await db.order.findOneAndUpdate(
            { '_id': ObjectId(result.out_trade_no) },
            { isPay: true, state: 2 },
            { upsert: true, new: true }
          )
        } else {
          console.log("His Recharge Err", rechargeResult)
          await db.order.findOneAndUpdate(
            { '_id': ObjectId(result.out_trade_no) },
            { isPay: true, state: 1 },
            { upsert: true, new: true }
      ...
      const resXml = await xmlUtil.json2xml({ return_code: 'SUCCESS', return_msg: 'OK' })
      ctx.body = { resXml }
    }
  }

  // 退款结果通知
  async refundNotifyAPI(ctx) {
    const xml = await new Promise((resolve, reject) => {
      try {
          let xmlData = ''
          ctx.req.on('data', (data) => {
    ...
    //  查询退款结果
    const result = await this.wechatRefundResult(payQuery)
    console.log('result', result)
    //  告知微信已收到，并调 HIS 接口扣款
    if (result.return_code && result.return_code == 'SUCCESS' && result.trade_state == 'SUCCESS' ) {
      const ChargeAmt = (await db.refundorder.findOne({ '_id': ObjectId(result.out_trade_no) })).refund_fee
      const HisID = (await db.refundorder.findOne({ '_id': ObjectId(result.out_trade_no) })).HisID
      const isRefund = (await db.refundorder.findOne({ '_id': ObjectId(result.out_trade_no) })).isRefund
      if (!isRefund) {
        const data = 
        `<![CDATA[
          <HisTrans>
          <TranCode>RefundToClinicCard</TranCode>
          <FtyCode>DM01</FtyCode>
          <DevNo></DevNo>
          <DevSeqNo></DevSeqNo>
          <Date>${ moment().format('YYYYMMDD') }</Date>
        ...
        const rechargeResult = await xmlUtil.handleXml(data, true)
        console.log('rechargeResult', rechargeResult)
        if (rechargeResult.RespCode === '0') {
          await db.refundorder.findOneAndUpdate(
            { '_id': ObjectId(result.out_trade_no) },
            { isRefund: true, state: 2 },
            { upsert: true, new: true }
          )
        } else {
          console.log("His Recharge Err", rechargeResult)
  ...
  //  门诊预交金充值记录查询
  async rechargeHisAccountHistoryAPI(ctx) {
  ...
  //  订单支付结果查询
  async getRechargeResultAPI(ctx) {
    const { _id, userId } = ctx.state.params
    ctx.body = await db.order.findOne({ _id, userId })
  }
  ...
  //  退款
  async refundAPI(ctx) {
    const { userId, HisID, sign } = ctx.state.params
    let { refund_fee } = ctx.state.params
    let data = _.omitBy(
      {
        userId,
        HisID,
    ...
    const orders = await db.order.find({ userId, HisID, isPay: true }).sort({ created: -1 })
    let orderTotalFee = 0
    orders.map(order => {
      orderTotalFee += order.total_fee
    })
    if (orderTotalFee < refund_fee) {
      throw createErr(400, '退款金额超过充值总金额，请重新核对')
    }
    ...
    for (const order of orders) {
      ...
      refund_fee > order.total_fee ? data.refund_fee = order.total_fee : data.refund_fee = refund_fee
      data.orderId = order._id
      const refundorder = await db.refundorder.create(data)
      ...
      let obj = {
        ...
        refund_fee: parseInt(refundorder.refund_fee * 100),
        notify_url: config.WECHAT.REFUND_NOTIFY_URL,
      }
  
      const refundArr = Object.keys(obj).sort().map(item => {
        return `${item}=${obj[item]}`
      })
      const refundStr = refundArr.join('&') + '&key=' + config.WECHAT.KEY
      obj.sign = await this.getSign(refundStr)
  
      try{
        // 调用微信退款接口
        const res = await this.wechatRefund(obj)
        console.log('res', res)
      }catch(err){
          console.log('Refund ERR', err)
      }
      refund_fee = new decimal(refund_fee).sub(new decimal(order.total_fee)).toNumber()
      frequency++
    }

    ctx.body = {msg: `用户您好！根据您的充值记录，此次退款分${frequency}次退回！`}
  }
  ...
  async wechatRefund(obj) {
    const xml = await xmlUtil.json2xml(obj)
    await request({
      url: config.WECHAT.REFUND_URL,
      method: 'POST',
      body: xml,
      agentOptions: {
          pfx: fs.readFileSync(__dirname + '../cert/apiclient_cert.p12'),
          passphrase: obj.mch_id
      }
    ...
    // const result = await axios.axios_post(
    //   config.WECHAT.REFUND_URL,
    //   xml
    // )
    // return (await xmlUtil.parseXml(result.data)).xml
  }

  async wechatRefundResult(obj) {
    const xml = await xmlUtil.json2xml(obj)
    const result = await axios.axios_post(
      config.WECHAT.REFUNDQUERY_URL,
      xml
    )
    return (await xmlUtil.parseXml(result.data)).xml
  }
  ...
  //  待入账费用查询
  async inquireRecordChargeAPI(ctx) {
    const { HisID } = ctx.state.params
    const data = 
      `<![CDATA[
        <HisTrans>
        <TranCode>ProjHistory</TranCode>
        <FtyCode>DM01</FtyCode>
        <DevNo></DevNo>
        <DevSeqNo></DevSeqNo>
        <Date></Date>
  ...
  //  费用结算列表
  async costSettlementListAPI(ctx) {
    const { HisID, QueryBeginDate, QueryEndDate } = ctx.state.params
    const data = 
      `<![CDATA[
        <HisTrans>
        <TranCode>GetMzCostList</TranCode>
        <FtyCode>DM01</FtyCode>
        <DevNo></DevNo>
        <DevSeqNo></DevSeqNo>
        <Date></Date>
