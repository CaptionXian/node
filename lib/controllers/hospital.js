...
class HospitalCtrl extends Base {
    ...
    async getQuestionnaireDate(conds) {
        let data = []
        let excelData = []
        const questionnaires = await db.questionnaires.find(conds).lean()
        for(let questionnaire of questionnaires) {
            data.push(questionnaire.name)
            data.push(questionnaire.tel)
            data.push(questionnaire.answer[0])
            data.push(questionnaire.answer[1])
            data.push(questionnaire.answer[2])
            data.push(questionnaire.answer[3])
            data.push(questionnaire.answer[4])
            data.push(questionnaire.answer[5])
            data.push(questionnaire.answer[6])
            data.push(questionnaire.answer[7])
            data.push(questionnaire.answer[8])
            data.push(questionnaire.answer[9])
