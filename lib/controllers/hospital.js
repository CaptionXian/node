...
class HospitalCtrl extends Base {
    ...
    async setWithDepartmentName(ctx) {
        ...
        for(let result of results) {
            result.departmentName = (await db.department.find({ _id: ObjectId(result.department) }, { title: 1 })).title
            console.log("vvv",result.department)
            console.log("vvv",result.departmentName)
        }
