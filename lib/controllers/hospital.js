...
class HospitalCtrl extends Base {
  ...
  //  寄送病历
  async sendMedicalRecordCopyAPI (ctx, next) {
    const { _id } = ctx.state.params
    const data = {
      isSend: true,
      updated: new Date()
    ...
    ctx.body = await db.record.findOneAndUpdate(
      { _id },
      data,
      { upsert: true, new: true }
    )
  ...
  async getOrderList (ctx) {
    const { orderId, state, startDate, endDate, userId } = ctx.state.params
    let conds = _.omitBy(
      {
        _id: orderId,
        userId,
    ...
    ctx.body = await db.order.find(conds)

    await jsonify(ctx)
    await this.setWithUserName(ctx)
  }
  ...
  //  附带用户信息
  async setWithUserName (ctx) {
    let results = ctx.body
    if (!Array.isArray(results)) {
      results = Array(results)
    }
    for (let result of results) {
      console.log('aaaa',result.userId)
      console.log('ssss',await db.wxuser.findOne({ _id: Object(result.userId) }, { nickname: 1 }))
      result.userName = (await db.wxuser.findOne({ _id: Object(result.userId) }, { nickname: 1 })).nickname
    }
  }
}
